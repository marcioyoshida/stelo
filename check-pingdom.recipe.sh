...
# Endpoint do API para verificar o pingdom recipes

API_API="https://api.pingdom.com/api/2.1/tms.recipes"

# LOCKFILE
LOCK="/tmp/api-pingdom-recipe.lck"

# TEMP File
TEMP_FILE="/tmp/dbpingdom-recipe.temp"

## Agente de envio
#ZBXPRX="zbxprx04.stelo.intranet"
ZBXPRX=`grep ServerActive /etc/zabbix/zabbix_agentd.conf | cut -d"=" -f2`
#ZBXPRX="zbxsrv01-homolog.stelo.local"
#ZBXPRX=localhost
...
export no_proxy='.stelo.local, 10.0.0.0/8'
...
# 		Extrai e lista o ID das recipes que estão com active = YES
        pingdom_RECIPE_LIST=`cat dbpingdom-recipe.temp | awk -v n=4 '/YES/ && NR>n {print window[(NR-n)%n]}{window[NR%n]=$0}' | sed -e 's/"//g' -e 's/,//g' -e 's/{//g' -e 's/://g'`
        for pingdom_test in `echo $pingdom_RECIPE_LIST`
                do
